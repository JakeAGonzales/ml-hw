...
def main():
    ...
    fig, fig_axes = plt.subplots(ncols=columns, nrows=rows, figsize=(columns*fig_scale, rows*fig_scale))

    for k in range(rows):
        for m in range(columns):
            idx = columns*k+m 
            image, label = (img[idx], f"Pred: {predicted[idx]}, actual: {actual[idx]}")

            ax = fig_axes[k][m]
            ax.set_axis_off()
            ax.set_title(label)
            ax.imshow(np.reshape(image, (28,28)))
    plt.show() 
